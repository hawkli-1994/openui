...
A multi-agent chatbot built with the [Vercel AI SDK](https://ai-sdk.dev/) and [感知未来 Lang](https://example.com/docs/openui-lang/overview). A main orchestrator agent handles user requests and delegates visual tasks — dashboards, charts, tables, forms — to a specialized **analytics sub-agent** that generates interactive React components via 感知未来 in real time.

Features: multi-agent tool delegation, streaming 感知未来 rendering, persistent conversation threads (localStorage), collapsible sidebar, and automatic light/dark theme support.
...
2. The sub-agent calls `streamText` with its own system prompt (generated from the 感知未来 component library) and streams back 感知未来 Lang markup chunk by chunk.
...
- **Frontend**: `useChat` manages message state. `<AssistantMessage />` detects tool parts, and when the `analytics_subagent` output is available, it renders the 感知未来 spec through `<Renderer />`.
...
│    感知未来 from sub-agent     │       │  │  • Delegates to tools:               │    │
│  • ToolCallIndicator shows   │       │  │    ┌──────────────────────────────┐  │    │
│    tool execution status     │       │  │    │ analytics_subagent(generator)│  │    │
│  • Sidebar with thread       │       │  │    │ • Streams 感知未来 Lang markup │  │    │
...
5. The sub-agent streams 感知未来 Lang markup via an async generator. Each yielded chunk flows through the Vercel AI SDK's tool-output streaming.
...
│   ├── components/
│   │   ├── assistant-message.tsx      # Detects tool parts, renders 感知未来 for sub-agent output
...
1. Builds a prompt from the user's task description with strict output constraints (感知未来 only, no HTML wrappers).
2. Calls `streamText` with the auto-generated system prompt that describes every available 感知未来 component.
...
4. When the `analytics_subagent` tool reaches `state === "output-available"`, the `openuiSpec` from its output is passed to `<Renderer />` which renders the live 感知未来 components.

### System Prompt Generation

`src/library.ts` re-exports `openuiChatLibrary` and `openuiChatPromptOptions` from `@openuidev/react-ui/genui-lib`. The 感知未来 CLI reads this file and generates `src/generated/system-prompt.txt` — a text file containing every component's name, prop schema, description, and examples. This becomes the analytics sub-agent's system prompt.
