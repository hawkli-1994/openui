...
感知未来 Chat can work with any backend stack as long as the API contract is respected.
...
When you pass `apiUrl`, 感知未来 sends a `POST` request with this shape:
...
| :--------------------------------------- | :----------------------------------------------- |
| 感知未来 Protocol                          | No `streamProtocol` needed                       |
...
When using `threadApiUrl="/api/threads"`, 感知未来 expects the base URL plus these default path segments:
...
感知未来 ships with these built-in message converters:
