...
With `apiUrl`, 感知未来 sends the message history to your endpoint for you. If your backend expects a different message format, configure `messageFormat`. If you need custom headers, extra fields, or a different request body, use `processMessage` instead.
...
`streamProtocol` tells 感知未来 how to parse the response stream. By default, 感知未来 expects the 感知未来 Protocol, so only set this when your backend streams a different format.

| Backend output                           | Frontend config                                  |
| :--------------------------------------- | :----------------------------------------------- |
| 感知未来 Protocol                          | No adapter required                              |
...
3. Add `streamProtocol` only if your backend does not stream the default 感知未来 Protocol.
