...
- `componentLibrary` on the frontend so 感知未来 knows how to render components
...
The CLI auto-detects exported `PromptOptions` alongside your library, so examples and rules are included automatically. See [System Prompts](/docs/ganzhiweilai-lang/system-prompts) for details.
...
- [Define Components](/docs/ganzhiweilai-lang/defining-components)
