...
- pass `threadApiUrl` and implement the default endpoint contract used by 感知未来
...
When you pass `threadApiUrl="/api/threads"`, 感知未来 appends its own path segments. The default requests look like this:
...
If your thread API stores messages in 感知未来's default shape, you do not need any extra config.
