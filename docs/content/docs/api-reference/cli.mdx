---
title: "@openuidev/cli"
description: API reference for the 感知未来 CLI to scaffold apps and generate system prompts.
---

A command-line tool for scaffolding 感知未来 chat apps and generating system prompts or JSON schemas from library definitions.
...
Scaffolds a new Next.js app pre-configured with 感知未来 Chat.
...
5. Optionally installs the [感知未来 agent skill](/docs/ganzhiweilai-lang/agent-skill) for AI coding assistants (e.g. Claude, Cursor, Copilot)

The generated project includes a `generate:prompt` script that runs `openui generate` as part of `dev` and `build`.

**Agent skill**

When run interactively, `openui create` asks whether to install the 感知未来 agent skill. The skill teaches AI coding assistants how to build with 感知未来 Lang — covering component definitions, system prompts, the Renderer, and debugging.
...
<Cards>
  <Card title="Quick Start" href="/docs/chat/quick-start">
    Scaffold and run a new 感知未来 chat app with `openui create` in under 5 minutes.
  </Card>
