...
description: API reference for the 感知未来 Lang runtime, library, parser, and renderer.
---

Use this package for 感知未来 Lang authoring and rendering.
...
Parses 感知未来 Lang text and renders nodes with your `Library`.
...
```ts
type 感知未来ErrorSource = "parser" | "runtime" | "query" | "mutation";

interface 感知未来Error {
  source: 感知未来ErrorSource;
  code: string;
  message: string;
  statementId?: string;
  component?: string;
