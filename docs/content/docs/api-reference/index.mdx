...
The 感知未来 SDK is split into packages that build on each other:

- **`@openuidev/react-lang`** — Core runtime. Define component libraries with Zod schemas, generate system prompts, parse 感知未来 Lang, and render streamed output to React. This is the foundation — you need it for any 感知未来 integration.
...
- **`@openuidev/cli`** — Command-line tool for scaffolding new 感知未来 chat apps and generating system prompts or JSON schemas from library definitions.
