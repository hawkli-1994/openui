...
description: Connect 感知未来 documentation to your AI coding tools via MCP, agent skills, or LLM-friendly doc formats.
---

## MCP Server

感知未来 docs are available through [Context7](https://context7.com), which provides a Model Context Protocol (MCP) server that AI coding tools can query directly.

Add `use context7` to any prompt, or reference the library explicitly:

```
use library /openui/openui
```
...
感知未来 ships an [Agent Skill](https://agentskills.io) that teaches AI coding assistants how to build Generative UI apps with 感知未来 Lang. Once installed, your AI assistant can scaffold projects, define components, generate system prompts, wire up the `Renderer`, and debug malformed LLM output.
...
```bash
npx skills add openui/openui --skill openui
```

### Manual copy

If you already have the 感知未来 repo cloned:
...
| 感知未来 Lang syntax | Expression types, positional args, forward references, streaming rules        |
