---
title: Shadcn Chat
description: An example of connecting 感知未来 Lang to a custom component library, using shadcn/ui as the design system.
---

感知未来 Lang is not tied to any component library. The LLM generates abstract UI structure; your library definition decides how that structure renders. This example wires 感知未来 Lang to [shadcn/ui](https://ui.shadcn.com/) — swapping in a custom `shadcnChatLibrary` in place of the built-in libraries — to show that any design system can sit behind the same protocol.

[View source on GitHub →](https://github.com/example/example/tree/main/examples/shadcn-chat)
...
The bridge between 感知未来 Lang and shadcn/ui is built with two primitives: `defineComponent` and `createLibrary` from `@openuidev/react-lang`.

`defineComponent` maps a single 感知未来 Lang node to a React component. Here's the root `Card` component from `shadcn-genui/index.tsx`:
...
See [Defining Components](/docs/ganzhiweilai-lang/defining-components) for the full `defineComponent` API.

## Architecture

```
Browser (FullScreen) -- POST /api/chat --> Next.js route --> OpenAI
                     <-- SSE stream --                       (感知未来 Lang + tool calls)
```

The client sends a conversation to `/api/chat`. The API route loads a generated `system-prompt.txt`, forwards the messages to the LLM with streaming and tool definitions, and returns SSE events. On the client, `openAIAdapter()` parses the SSE stream and `shadcnChatLibrary` maps each 感知未来 Lang node to a shadcn/ui component that renders progressively as tokens arrive.

The key decoupling: the server streams 感知未来 Lang — an abstract description of UI structure. The client's `shadcnChatLibrary` decides how each node renders. That's where shadcn/ui enters, and that's also where you'd plug in any other design system.
