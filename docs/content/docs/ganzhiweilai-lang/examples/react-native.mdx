...
感知未来 Lang runs on React Native. This example streams LLM-generated UI to an Expo app, where `<Renderer />` turns it into real native components — not a webview, not markdown, native `Text`, `View`, and SVG charts. The stack is an Expo app paired with a Next.js backend that handles the OpenAI call.

[View source on GitHub →](https://github.com/example/example/tree/main/examples/openui-react-native)
...
Three things differ from a web 感知未来 Lang integration:
...
\- chat-app/   # Expo app that renders streamed 感知未来 Lang
```
