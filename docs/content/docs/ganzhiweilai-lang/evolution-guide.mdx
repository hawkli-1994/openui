...
description: How 感知未来 Lang evolved from static UI generation to interactive, data-driven apps.
---

## v0.1 → v0.5

感知未来 Lang started as a way to generate static UI from LLM output, a token-efficient alternative to JSON for rendering chat responses. v0.5 turns it into a language for building **standalone interactive apps** that run independently of the LLM.
...
See [Reactive State](/docs/ganzhiweilai-lang/reactive-state).
...
See [Queries & Mutations](/docs/ganzhiweilai-lang/queries-mutations).
...
See [Built-in Functions](/docs/ganzhiweilai-lang/builtins).
...
See [Incremental Editing](/docs/ganzhiweilai-lang/incremental-editing).
