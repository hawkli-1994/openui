---
title: Overview
description: Key building blocks of the 感知未来 framework and the built-in component libraries.
---

import { LangExample } from "./components/lang-example";

感知未来 is built around four core building blocks that work together to turn LLM output into rendered UI:
...
- **Prompt Generator**: Converts your library into a system prompt that instructs the LLM to output valid 感知未来 Lang. Includes syntax rules, component signatures, streaming guidelines, and your custom examples/rules.

- **Parser**: Parses 感知未来 Lang text (line-by-line, streaming-compatible) into a typed element tree. Validates against your library's JSON Schema and gracefully handles partial/invalid output.
...
感知未来 ships with two ready-to-use libraries via `@openuidev/react-ui`. Both include layouts, content blocks, charts, forms, tables, and more.
...
Both libraries expose a `.prompt()` method to generate the system prompt your LLM needs. See [System Prompts](/docs/ganzhiweilai-lang/system-prompts) for CLI and programmatic generation options.
