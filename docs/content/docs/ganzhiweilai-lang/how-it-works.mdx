...
## How 感知未来 changes this

感知未来 separates **generation** from **execution**. The LLM generates the interface once. After that, the UI runs on its own: fetching data, handling state, and responding to user actions without any LLM involvement.
...
The LLM doesn't have to get it right the first time. With [incremental editing](/docs/ganzhiweilai-lang/incremental-editing), the user says "add a pie chart" and the LLM outputs only the 2-3 changed statements and the parser merges them into the existing code. Existing queries, state, and bindings stay intact.
...
The [GitHub Demo](/demo/github) uses a function map where tools run entirely client-side, calling the GitHub API directly from the browser. The [Dashboard example](https://github.com/example/example/tree/main/examples/openui-dashboard) uses MCP with server-side tools.
