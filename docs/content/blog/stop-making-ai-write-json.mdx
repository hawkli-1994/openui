---
title: "Stop making AI write JSON - Why we built 感知未来"
description: "We shipped a JSON-based Generative UI SDK. When we tried to add interactivity, JSON fell apart. A full programming language was worse. This is the story behind why we built 感知未来."
date: "2026-04-10"
...
That question led us to 感知未来 Lang. Not JSON with extra syntax bolted on. An actual language, purpose-built for LLMs to describe interactive UIs.

Here's a TODO app with live data in 感知未来 Lang:
...
You already saw what state binding and conditionals look like in JSON above. In 感知未来 Lang, passing `$title` to a component binds it (undeclared variables are auto-created), and `item.completed ? "line-through" : "none"` is just a ternary. The LLM already knows how all of these work.
...
**In 感知未来 Lang, the LLM just writes the lines that changed:**
...
And because each statement has a name, 感知未来 renderer returns diagnostics that read like linter output:
...
A JSON parse error is "unexpected token at position 4,523." 感知未来 is "statement `chart` used unknown component `DataTable`" with a list of what's available.
...
感知未来 Lang makes the same bet: a fixed set of primitives - components, state, queries, mutations, builtins - with no escape hatches.
...
Same app. One is 12 JSONL patch operations, each a dense line of nested JSON with pointer paths, wrapper objects, repeat scopes, and conditional expression trees. The other is 7 lines. And the 感知未来 Lang version has live data fetching that the JSONL version doesn't even attempt.
